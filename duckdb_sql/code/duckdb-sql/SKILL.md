...
## Consolidate/Package Dataset

When users want to create a deliverable package of their data, use this workflow.

**Trigger phrases:** "consolidate this data set", "package up this data set", "package this for delivery", "create a snapshot", "bundle the databases"

**What it does:** Creates a new subdirectory containing a single `.ddb` file with all tables from all source files, plus updated documentation assets. Original files remain untouched.

### Workflow

1. **Verify prerequisites:**
   - Check that `duckdb_sql_assets/` exists with `tables_inventory.json`
   - If not: "No DuckDB assets found. Please run initial setup first."

2. **Detect table name conflicts:**
   - Read `tables_inventory.json` and check for duplicate table names across source files
   - If the same table name exists in multiple source files, present conflict resolution options (see below)

3. **Prompt for configuration:**

   > **Package Configuration:**
   >
   > I'll create a consolidated package with all your tables in a single database.
   >
   > - **Output directory:** `packaged/` (in current working directory)
   > - **Database filename:** `<derived_name>.ddb` (derived from parent directory: "project_name")
   >
   > Does this look correct? You can specify different names for the directory or database file.

4. **Create the package:**
   - Create the output subdirectory
   - Create `duckdb_sql_assets/` within it
   - Create consolidated `.ddb` file using DuckDB ATTACH + CREATE TABLE AS SELECT
   - Generate new assets (see "Asset Generation" below)
...
5. **Report completion:**
   ...
   >
   > - Location: `./packaged/`
   > - Database: `project_name.ddb` (X tables)
   > - Assets: `duckdb_sql_assets/` with updated documentation
   >
   > The original database files remain untouched.

### Table Name Conflict Resolution

If the same table name exists in multiple source files:

> **Table name conflict detected:**
>
> The table `users` exists in both `sales.ddb` and `support.ddb`.
>
> **Options:**
> 1. **Prefix with source name** (recommended): Creates `sales_users` and `support_users`
> 2. **Skip duplicates:** Keep only the first occurrence (from sales.ddb)
> 3. **Abort:** Cancel and resolve manually
>
> Which approach should I use?

Default to the prefix strategy if user doesn't specify.

### DuckDB Commands for Consolidation

```sql
-- Create new database and attach sources as read-only
ATTACH '/path/to/sales.ddb' AS src_sales (READ_ONLY);
ATTACH '/path/to/inventory.ddb' AS src_inv (READ_ONLY);

-- Copy each table (preserves schema and data)
...
-- If using prefix for conflicts:
CREATE TABLE sales_users AS SELECT * FROM src_sales.users;
CREATE TABLE support_users AS SELECT * FROM src_support.users;
...
```json
{
  ...
  "packaged_from": {
    "original_sources": ["sales.ddb", "inventory.ddb"],
    "packaged_at": "2025-12-11T15:30:00Z"
  },
  "sources": [
    {
      "file_path": "./project_name.ddb",
      "file_name": "project_name.ddb",
      "tables": ["customers", "orders", "products", "stock"]
    }
  ],
  "tables": {
    "customers": { "source_file": "project_name.ddb", "columns": [...] },
    "orders": { "source_file": "project_name.ddb", "columns": [...] },
    "products": { "source_file": "project_name.ddb", "columns": [...] },
    "stock": { "source_file": "project_name.ddb", "columns": [...] }
  }
}
```

**data_dictionary.md:**
- Update header: `**Source Files:** project_name.ddb`
- Add provenance: `**Packaged from:** sales.ddb, inventory.ddb (YYYY-MM-DD)`
- Update each table's `**Source file:**` to the new filename
- Add `**Original source:**` line to preserve provenance
- If tables were renamed due to conflicts, update names and add to "Also known as"

**schema_<name>.sql:**
- Single schema file generated from consolidated database
- Named after the database: `schema_project_name.sql`
...
This is a packaged snapshot created on YYYY-MM-DD.
**Packaged from:** sales.ddb, inventory.ddb
